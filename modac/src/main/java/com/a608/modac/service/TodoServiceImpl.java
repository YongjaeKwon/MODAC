...
public class TodoServiceImpl implements TodoService {
	...
	public TodoResponse saveTodo(final TodoRequest todoRequest) {
		// Validation
		User user = userRepository.findById(todoRequest.getUsersSeq())
			.orElseThrow(() -> new NoSuchElementException("NoUser"));
		Category category = categoryRepository.findByName(todoRequest.getCategoriesName());
		if (category == null) {
			throw new NoSuchElementException("NoCategory");
		}
